...
class DdmmToLemmaTransformationHandler extends AbstractHandler {
    ...
    static val TRACE_MODEL_NAME = "trace"
    static val TRACE_MODEL_FILE_NAME = "trace.xmi"
    ...
    private def domainTransformations(String context, Resource umlResource, IProject project) {
        ...
        val resultModels = atlTransformation(
        ...
        var resultModel = resultModels.get("INOUT")
        ...
        // Serialize trace model
        /*if (context.empty) {
            val traceModel = resultModels.get(TRACE_MODEL_NAME)
            traceModel.serialize(project,
                '''«targetFolder»«File.separator»domain_«TRACE_MODEL_FILE_NAME»''')
        }*/
    ...
    private def atlTransformation(String moduleBasePath, String moduleName, List<AtlParam> inModels,
        ...
        /*val traceModel = EmftvmFactory.eINSTANCE.createModel()
        traceModel.resource = buildTransformationResourceSet.createResource(
            URI.createFileURI(TRACE_MODEL_FILE_NAME)
        )
        env.registerInOutModel(TRACE_MODEL_NAME, traceModel)
        models.put(TRACE_MODEL_NAME, traceModel.resource)*/
    ...
    private def copyTo(IFile source, String targetFileName, boolean isDerived) {
        ...
        if (!target.exists)
            target.create(new ByteArrayInputStream(newByteArrayOfSize(0)), IResource.NONE,
