...
class DataDslExtractor extends AbstractGenerator {
	static val FEATURE_TO_TEXT_MAPPINGS = #{
		...
		ComplexTypeFeature.APPLICATION_SERVICE -> "applicationService",
		ComplexTypeFeature.DOMAIN_EVENT -> "domainEvent",		
		ComplexTypeFeature.DOMAIN_SERVICE -> "domainService",		
		ComplexTypeFeature.ENTITY -> "entity",
		ComplexTypeFeature.FACTORY -> "factory",
		ComplexTypeFeature.INFRASTRUCTURE_SERVICE -> "infrastructureService",
		ComplexTypeFeature.REPOSITORY -> "repository",
		ComplexTypeFeature.SERVICE -> "service",
		ComplexTypeFeature.SPECIFICATION -> "specification",
		ComplexTypeFeature.VALUE_OBJECT -> "valueObject",
	...
	private def dispatch generateComplexTypeDefinition(Enumeration enumeration) {
		return '''
