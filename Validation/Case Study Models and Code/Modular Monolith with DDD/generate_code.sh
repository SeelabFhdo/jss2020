...
VALIDATION_NAME="Modular Monolith with DDD"
services=(
    Administration
    Meetings
    Payments
...
generate_code() {
    SERVICE=$1
    eval "java -jar libs/de.fhdo.lemma.model_processing.code_generation.java_base-0.7.0-SNAPSHOT-standalone.jar -s \"$VALIDATION_NAME/Phase 4/$SERVICE.mapping\" -i \"$VALIDATION_NAME/intermediate/mapping models/$SERVICE.xmi\" -t \"$VALIDATION_NAME/generated code\" code_generation main --code_generation_serializer=$PATTERN \"--genlet=libs/de.fhdo.lemma.model_processing.code_generation.springcloud-0.7.0-SNAPSHOT.jar\" \"--genlet=libs/de.fhdo.lemma.model_processing.code_generation.springcloud.domain_events-0.7.0-SNAPSHOT.jar\" \"--genlet=libs/de.fhdo.lemma.model_processing.code_generation.springcloud.kafka-0.7.0-SNAPSHOT.jar\""
}

compile() {
    SERVICE="$(echo "$1" | sed -e 's/\(.*\)/\L\1/')"
...
mvn install:install-file -Dfile=../libs/de.fhdo.lemma.msa-0.7.0-SNAPSHOT.jar -DgroupId=de.fhdo.lemma.msa -DartifactId=de.fhdo.lemma.msa -Dversion=0.7.0-SNAPSHOT -Dpackaging=jar
