...
DOMAIN_ADAPTATION_REGEX = re.compile('\s*//\s*(?P<id>D\d)')
SERVICE_ADAPTATION_REGEX = re.compile('\s*//\s*(?P<id>S\d)')
MANDATORY_ADAPTATIONS = ["D3", "S3", "S4", "S5"]
...
def find_adapations(file):
    ...
    with open(file,'r') as fd:
        for line in fd.readlines():
            match = DOMAIN_ADAPTATION_REGEX.match(line)
            if not match:
                match = SERVICE_ADAPTATION_REGEX.match(line)
                if not match:
                    continue
...
def print_adaptation_counts(adaptationCounts, prefix='\t'):
    ...
    for id, count in sorted(adaptationCounts.items(), key=lambda i: i[0]):
        print('%s%s: %s' % (prefix, id, count))
        sum += count
        if id in MANDATORY_ADAPTATIONS:
            mandatorySum += count
